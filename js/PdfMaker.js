app.service('PdfMaker', [function() {

    //this.createChart = function(dob,age,fy,cses,thp,resultWithoutSS,resultWithSS,needSS,optimisedSS,toggleNeeded){
    this.createChart = function(dob, age, genderOption, spouseOption, numChildren,assetsObject, liabilitiesObject,otherExpenses , existingCovers, assumptions, resultS1, resultS2) {

        var cdob = dob.toString().split(" ")[1] + " " + dob.toString().split(" ")[2] + " " + dob.toString().split(" ")[3];
        console.log(resultS1.trauma);
        var moneyFormat = wNumb({
            mark: '.',
            thousand: ',',
            prefix: '$',
            decimals: 2
                // postfix: ' p.p.'
        });

        var gender = genderOption ? "Male" : "Female";

        var spouse = spouseOption ? "Yes" : "No";

        var saving;

        if (true) {
            saving = moneyFormat.to(1000);
        } else {
            saving = moneyFormat.to(1000)
        };

        var imgData2 = 'data:image/png;base64,';

        var columnsP1 = [
            { title: "Insurance Adequacy Calculations", dataKey: "info"},
        ];

        var columnsP2 = [
            { title: "Comparison Graphs", dataKey: "info"},
        ];

        var columnsP4 = [
            { title: "Final Results", dataKey: "info"},
        ];



        var columnsP3 =[{ title: "Calculator Information", dataKey: "info"},];

        var rowsP3 =[
        { "info": "1) The total estimated level of Death cover you need is calculated by taking two times your pre-tax annual income plus an amount for each child under 18 years of age plus debt repayments plus funeral costs plus other expenses minus the total of your assets (including any existing death cover)."},
        { "info": "2) The total estimated level of Total and Permanent Disablement cover you need is calculated by taking two times your pre-tax annual income plus an amount for each child under 18 years of age plus your current debt plus other expenses minus the total of your assets (including any existing TPD cover)."},
        { "info": "3) The total estimated level of Trauma cover you need is based on the total of your current debt reduced by any emergency funds and any existing Trauma cover."},
        ];

        var columns1 = [
            { title: "General Information", dataKey: "info"},
            { title: "Values", dataKey: "value" },
        ];
        var rows1 = [
            { "info": "Date Of Birth", "value": cdob},
            { "info": "Age", "value": age },
            { "info": "Gender", "value": gender },
            { "info": "Do You Have A Spouse", "value": spouse },
            { "info": "Number Of Children", "value": numChildren },
        ];

        var columns2 = [
            { title: "Assets", dataKey: "assets" },
            { title: "Value", dataKey: "value" },
        ];
        var rows2 = [
            { "assets": "Home Value", "value": assetsObject.homeValue},
            { "assets": "Cash At Bank", "value": assetsObject.cashAtBank},
            { "assets": "Other Investments", "value": assetsObject.otherInvestment},
            { "assets": "Super Balance", "value": assetsObject.superBalance},
        ];

        var columns3 = [
            { title: "Liabilities", dataKey: "liabilities" },
            { title: "Value", dataKey: "value" },
        ];
        var rows3 = [
            { "liabilities": "Home Mortgage", "value": liabilitiesObject.homeMortgage},
            { "liabilities": "Investment Property Mortgage", "value": liabilitiesObject.investmentPropertyMortgage},
            { "liabilities": "Credit Card Debt", "value": liabilitiesObject.creditCardDebt},
            { "liabilities": "Car Loan", "value": liabilitiesObject.carLoan},
            { "liabilities": "Personal Loan", "value": liabilitiesObject.personalLoan},
            { "liabilities": "Other Loan", "value": liabilitiesObject.otherLoan},
        ];

        var columns4 = [
            { title: "Other Expenses", dataKey: "expenses" },
            { title: "Value", dataKey: "value" },
        ];
        var rows4 = [
            { "expenses": "Funeral Cost", "value": otherExpenses.funeralCost},
            { "expenses": "Education Expense Per Year Per Child", "value": otherExpenses.educationExpense},
            { "expenses": "Family Living Cost", "value": otherExpenses.familyLivingCost},
            { "expenses": "In the event of death, is your spouse considering to move to a smaller property?", "value": otherExpenses.moveProperty},
            { "expenses": "Value Of The Property You Want To Buy", "value": otherExpenses.newPropertyValue},
            { "expenses": "Money Your Spouse Would Like To Borrow", "value": otherExpenses.moneyBorrowed},
            { "expenses": "Sale Proceeds of existing Property", "value": otherExpenses.saleProceeds},
        ];

        var columns5 = [
            { title: "Existing Cover", dataKey: "cover" },
            { title: "Value", dataKey: "value" },
        ];
        var rows5 = [
            { "cover": "Life Cover", "value": existingCovers.ecLife},
            { "cover": "TPD Cover", "value": existingCovers.ecTPD},
            { "cover": "Income Protection Cover", "value": existingCovers.ecIP},
            { "cover": "Trauma Cover", "value": existingCovers.ecTrauma},
        ];

        var columns6 = [
            { title: "Assumptions", dataKey: "assumption" },
            { title: "Value", dataKey: "value" },
        ];
        var rows6 = [
            { "assumption": "Wage/Living Cost Inflation", "value": assumptions.inflation},
            { "assumption": "Rate Of Return", "value": assumptions.rateOfReturn},
            { "assumption": "Real Rate Of Return", "value": assumptions.realRate},
        ];

        var columns7 = [
            { title: "Covers", dataKey: "cover"},
            { title: "Estimated Value", dataKey: "est" },
            { title: "Excess/Shortfall", dataKey: "excess" },
        ];
        var rows7 = [
            { "cover": "Death Cover", "est": 0,"excess":0},
            { "cover": "TPD Cover", "est": 0 ,"excess":0},
            { "cover": "Income Protection Cover", "est": 0 ,"excess":0},
            { "cover": "Trauma Cover", "est": 0,"excess": 0},
        ];

        var options = {
            theme: 'grid'
                // margin: {top: 380}
        };

        var doc = new jsPDF('p', 'pt');

        // doc.setFontSize(20);

        // doc.text(40, 30, "Insurance Adequacy Calculations");

        doc.autoTable(columnsP1,[], {
            margin: { top: 20 },
            styles:{
              rowHeight:40,
              halign : 'left',
              valign : 'middle',
              fontSize : 15
            }
        });

        doc.autoTable(columns1, rows1, {
            margin: { top: 85 },
            styles:{
              // fontSize:20,
            halign : "left",
            // columnWidth : 400
            },
            columnStyles:{
            info: {columnWidth: 450},
            value: {columnWidth: 67}
            },
        });

        doc.autoTable(columns2, rows2, {
            margin: { top: 235 },
            columnStyles:{
            info: {columnWidth: 450},
            value: {columnWidth: 67}
            },
        });

        doc.autoTable(columns3, rows3, {
            margin: { top: 355 },
            styles:{
              // fontSize:20,
            halign : "left",
          },
          columnStyles:{
            info: {columnWidth: 450},
            value: {columnWidth: 67}
            },
        });

        doc.autoTable(columns4, rows4, {
            margin: { top: 525 },
            columnStyles:{
            info: {columnWidth: 450},
            value: {columnWidth: 67}
            },
        });


        // doc.autoTable(columns6, rows6, {
        //     margin: { top: 680 },
        //     columnStyles:{
        //     info: {columnWidth: 450},
        //     value: {columnWidth: 67}
        //     },
        // });  
        // doc.addPage();

        doc.addImage(imgData2,'PNG',40,780);
        doc.setFontSize(10);
        doc.text(510,810,'PAGE ' + 1);       

        doc.addPage();

        doc.autoTable(columns5, rows5, {
            margin: { top: 20 },
            columnStyles:{
            info: {columnWidth: 450},
            value: {columnWidth: 67}
            },
        }); 

        doc.autoTable(columnsP2,[], {
            margin: { top: 145 },
            styles:{
              rowHeight:40,
              halign : 'left',
              valign : 'middle',
              fontSize : 15
            }
        });

        var canvas = document.createElement("canvas");

        canvg(canvas, $('#containerR').highcharts().getSVG());

        var img = canvas.toDataURL("image/png");

        canvg(canvas, $('#containerR2').highcharts().getSVG());

        var img2 = canvas.toDataURL("image/png");

        canvg(canvas, $('#containerR3').highcharts().getSVG());

        var img3 = canvas.toDataURL("image/png");

        canvg(canvas, $('#containerR4').highcharts().getSVG());

        var img4 = canvas.toDataURL("image/png");

        doc.addImage(img, 'PNG', 50, 215);

        doc.addImage(img2, 'PNG', 300, 215);

        doc.addImage(img3, 'PNG', 50, 510);

        doc.addImage(img4, 'PNG', 300, 510);

        doc.addImage(imgData2,'PNG',40,780);
        doc.setFontSize(10);
        doc.text(510,810,'PAGE ' + 3); 

        doc.addPage();

          doc.autoTable(columnsP4,[], {
            margin: { top: 20 },
            styles:{
              rowHeight:36,
              halign : 'left',
              valign : 'middle',
              fontSize : 15
            }
        });

        doc.autoTable(columns7,rows7,{
            margin: { top: 60 },
            styles:{
            //   rowHeight:40,
            //   halign : 'left',
            //   valign : 'middle',
            //   fontSize : 15
            // overflow : 'linebreak',
            // fontSize : 10
            }
        });

        doc.autoTable(columnsP3,rowsP3,{
            margin: { top: 220 },
            styles:{
            //   rowHeight:40,
            //   halign : 'left',
            //   valign : 'middle',
            //   fontSize : 15
            overflow : 'linebreak',
            fontSize : 10
            }
        });

        doc.addImage(imgData2,'PNG',40,780);
        doc.text(510,810,'PAGE ' + 3);


        doc.save('InsuranceAdequacyCalculation.pdf');

    }
}]);

//   var svgElements = $("#container").find('svg');
//   var svgElements2 = $("#container2").find('svg');

//   // svgElements.push(svgElements2[0]);

//   console.log(svgElements);

// //replace all svgs with a temp canvas
// svgElements.each(function() {
//   var canvas, xml;

//   // canvg doesn't cope very well with em font sizes so find the calculated size in pixels and replace it in the element.
//   $.each($(this).find('[style*=em]'), function(index, el) {
//     $(this).css('font-size', getStyle(el, 'font-size'));
//   });

//   canvas = document.createElement("canvas");
//   canvas.className = "screenShotTempCanvas";
//   //convert SVG into a XML string
//   xml = (new XMLSerializer()).serializeToString(this);

//   // Removing the name space as IE throws an error
//   xml = xml.replace(/xmlns=\"http:\/\/www\.w3\.org\/2000\/svg\"/, '');

//   //draw the SVG onto a canvas
//   canvg(canvas, xml);
//   $(canvas).insertAfter(this);
//   //hide the SVG element
//   ////this.className = "tempHide";
//   $(this).attr('class', 'tempHide');
//   $(this).hide();
// });






// svgElements2.each(function() {
//   var canvas2, xml2;

//   // canvg doesn't cope very well with em font sizes so find the calculated size in pixels and replace it in the element.
//   $.each($(this).find('[style*=em]'), function(index, el) {
//     $(this).css('font-size', getStyle(el, 'font-size'));
//   });

//   canvas2 = document.createElement("canvas2");
//   canvas2.className = "screenShotTempCanvas";
//   //convert SVG into a XML string
//   xml2 = (new XMLSerializer()).serializeToString(this);

//   // Removing the name space as IE throws an error
//   xml2 = xml2.replace(/xmlns=\"http:\/\/www\.w3\.org\/2000\/svg\"/, '');

//   //draw the SVG onto a canvas2
//   canvg(canvas2, xml2);
//   $(canvas2).insertAfter(this);
//   //hide the SVG element
//   ////this.className = "tempHide";
//   $(this).attr('class', 'tempHide');
//   $(this).hide();
// });


//       html2canvas($("#container"), {
//         onrendered: function(canvas) {
//           var imgData = canvas.toDataURL(
//             'image/png');
//           // var imgData3 = canvas.toDataURL(
//             // 'image/png');
//           var imgData2 = 'data:image/png;base64,';



//           var doc = new jsPDF('p', 'pt');
//           doc.setFontSize(20);
//           doc.text(40, 30, "Salary Sacrifice Optimisation");
//           doc.addImage(imgData2,'PNG',480,20);
//           // doc.setFontSize(10);
//           doc.addImage(imgData, 'PNG', 150, 225);
//           // doc.addImage(imgData3, 'PNG', 150, 225);
//           // doc.addImage(imgData4, 'PNG', 150, 225);
//           // doc.addImage(imgData5, 'PNG', 150, 225);
//           doc.autoTable(columns1,rows1,{
//             margin:{top:50},
//           });
//           doc.autoTable(columns2,rows2,{
//             margin:{top:580},
//           });
//           doc.autoTable(columns3,[],{
//             margin:{top:700},
//             styles:{
//             fontSize : 14,
//             overflow:'linebreak',
//             valign : 'middle',
//             fillColor : 255,
//             textColor : 0
//             // cellPadding : 10
//           }
//           });
//           // doc.addImage(imgData2,'PNG',40,780);
//           doc.text(510,810,'PAGE ' + 1);
//           doc.addPage();
//           doc.setFontSize(16);
//           doc.text(40, 30, "Calculator Information");
//           doc.line(40,40,560,40);
//           doc.setFontSize(12);
//           doc.text(40,55,"1) Salary indicated here does not include superannuation guarantee contribution, fringe benefit or\nnon-standard agreed employment benefit,  and is the salary before any salary sacrifice.");
//           doc.text(40,85,"2) SGC will gradually increase to 12.00% in the future.");
//           doc.text(40,100,"3) It is assumed that at least the minimum take-home(after salary sacrifice and tax) wage is required\nby the member.");
//           doc.text(40,130,"4) It is assumed that the superannuation balance is boosted up by the net concessional contribution\nin the financial year.");
//           doc.text(40,160,"5) This calculator takes into the consideration of both the personal tax and concessional contribution\ntax charged from the super.");
//           doc.text(40,190,"6) It is assumed that the member will contribute (including superannuation guarantee contribution\nand salary sacrifice) up to the maximum concessional contribution cap in a financial year.");
//           // doc.addImage(imgData2,'PNG',40,780);
//           doc.setFontSize(10);
//           doc.text(510,810,'PAGE ' + 2);
//           // doc.text(270,825,'PAGE ' + 2);
//           doc.save('SalarySacrificeOptimisation.pdf');
//         }
//       });

//       $("#container").find('.screenShotTempCanvas').remove();
//       $("#container").find('.tempHide').show().removeClass('tempHide');

//     //   if(toggleNeeded){
//     //   // document.getElementById("container").classList.toggle("ng-hide");
//     // }

// }

// }]);


// $('#download').click(function() {

//     });



// function getStyle(el, styleProp) {
//   var camelize = function(str) {
//     return str.replace(/\-(\w)/g, function(str, letter) {
//       return letter.toUpperCase();
//     });
//   };

//   if (el.currentStyle) {
//     return el.currentStyle[camelize(styleProp)];
//   } else if (document.defaultView && document.defaultView.getComputedStyle) {
//     return document.defaultView.getComputedStyle(el, null)
//       .getPropertyValue(styleProp);
//   } else {
//     return el.style[camelize(styleProp)];
//   }
// }




// Only pt supported (not mm or in)
// var doc = new jsPDF('p', 'pt');
// doc.autoTable(columns, rows, {
//     styles: {fillColor: [100, 255, 255]},
//     columnStyles: {
//         id: {fillColor: 255}
//     },
//     margin: {top: 60},
//     beforePageContent: function(data) {
//         doc.text("Header", 40, 30);
//     }
// });
// doc.save('table.pdf');
