...
this.createChart = function(dob,age,genderOption,spouseOption,numChildren,resultS1,resultS2){
  ...
  if(true){
    ...
    { "name": "Desired Minimum Take Home Salary Per Annum", "country":moneyFormat.to(resultS1.trauma)},
    ...
    var rows2 = [
          { "name": "Take Home Money", "wss":moneyFormat.to(1000), "ss":moneyFormat.to(1000)},
          { "name": "Accumulation End Balance", "wss":moneyFormat.to(1000), "ss":moneyFormat.to(1000)},
          { "name": "Final Amount", "wss":moneyFormat.to(1000), "ss":moneyFormat.to(1000)},
          ];
          
        if(true){ 
           var columns3 = [
          {title: "You save " + saving + " with salary sacrifice of " + moneyFormat.to(1000) + "." , dataKey: "name"},  
          ];
        }else{
          var columns3 = [
    ...
    var options = {
      ...
      doc.addImage(img, 'PNG', 50, 225);

      doc.addImage(img2, 'PNG', 300, 225);

      doc.addImage(img3, 'PNG', 50, 555);

      doc.addImage(img4, 'PNG', 300, 555);
